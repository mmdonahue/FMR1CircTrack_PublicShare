...
rewLocOutBnd = 15; % +/- rewLocOutBnd degrees --  around reward location --> This helps us ensure the start/stop were still close to the reward loc (between rewLocInBnd & rewLocOutBnd)
            ...
            for b = 1:length(group(g).rat(r).day(d).begin)
                ...
                for c = 1:length(lapChnks.PixelIdxList)
                    ...
                    if length(lapInds) > 20
                        ...
                        if rad2deg(circ_dist(deg2rad(min(outerBnds)),deg2rad(startLoc))) <= rewLocInBnd && ...
                                 rad2deg(circ_dist(deg2rad(endLoc), deg2rad(max(outerBnds)))) <= rewLocInBnd 
%                                 rad2deg(circ_dist(deg2rad(min(outerBnds)),deg2rad(startLoc))) > 0 && ...
%                             rad2deg(circ_dist(deg2rad(endLoc), deg2rad(max(outerBnds)))) > 0 && ...
