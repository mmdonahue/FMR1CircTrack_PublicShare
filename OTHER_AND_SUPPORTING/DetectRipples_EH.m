function [RippleOnsetIndex, RippleOffsetIndex, shamp, threshold, filtEEG, RipplePower]=DetectRipples_EH(EEG_all,sampfreq,varargin)
% function [RippleOnsetIndex, RippleOffsetIndex, shamp, threshold, filtEEG, RipplePower]=DetectRipples_EH(EEG_all, sampfreq, n_std, duration_threshold, time_kernel)
% 
% INPUTS:
%   EEG_all = n x 1 cell array with EEG from all CA1 tetrodes
%   sampfreq = sampling frequency of LFP
%   n_std = number of standard (optional - default = 3)
%   duration_threshold = minimum event duration (optional - default = 15 ms)
%   time_kernel = kernel used to smooth time series (optional - default = 25 ms)
% 
% OUTPUTS:
%   RippleOnsetIndex = onset indices (inds from LFP)
%   RippleOffsetIndex = offset indices (inds from LFP)
% 
...
if nargin > 3
    duration_threshold = varargin{2};
end


if nargin > 4
    time_kernel = sampfreq*varargin{3};
end
shamp = [];
%filter EEG
sampFreq=sampfreq;
...
for ee = 1:size(EEG_all,1)
	...
	xmax = find_peaks(RippleAmp(ee,:),threshold(ee));
