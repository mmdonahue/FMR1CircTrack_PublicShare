function h = dotplot(xVals, yData, jitter, faceColors, edgeColors, varagin)
% function h = dotplot(xVals, yData, jitter, faceColors, edgeColors, alphaLevel)
%
% PURPOSE:
...
%   alphaLevel = (optional) transparancy level for dots (default is 1)
%
% OUTPUT:
...
for g = 1:length(xVals) %For each group
    ...
    if exist('edgeColors', 'var')==1  &&  ~isempty(edgeColors)
       ...
       set(h(g), 'MarkerEdgeAlpha', alphaLevel);
       set(h(g), 'MarkerFaceAlpha', alphaLevel);
end
